...
> 当前版本：`0.2.3` 真机修正版。采集过程和结果都在原订单页显示临时横幅；从最近任务手动返回 App 时也会重新加载最新结果，不依赖 HyperOS 的后台跳转或延迟 Toast。抖音页面会强制补充内存 OCR，并从结构化结果中排除直播推荐。Mac 接收端自动化测试和 Android 单元测试已通过；真机上的六个购物 App 验收继续在小米 17 上完成。
